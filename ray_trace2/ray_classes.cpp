...
void Poly3D::print()
{ 
   cout << "Number of points: " << point.size() << endl;
...
{ 
   ...
   for (int i=0; i<point.size(); i++)
   {
      ...
      int i2 = (i+1) % point.size();
      v1.set(point[i2].px - point[i].px,  
             point[i2].py - point[i].py,  
             point[i2].pz - point[i].pz);

      // Vector from point i to pt
      v2.set(pt.px - point[i].px,  
             pt.py - point[i].py,  
             pt.pz - point[i].pz);
 
      // Check sign of z component of cross product
      if ((v1.vx * v2.vy - v1.vy * v2.vx) > 0) return false;
   }
   return true;
}

//----------------------------------------------
void Poly3D::add_point(Point3D pt)
{ 
   point.push_back(pt);
}
