...
unsigned char *yellow_texture = NULL;
unsigned char *gold_texture = NULL;
unsigned char *gems_texture = NULL;
int xdim = 0;
int ydim = 0;
...
void init()
{
   ...
   // Init texture
   init_texture((char *)"brick.jpg", brick_texture, xdim, ydim);
   init_texture((char *)"fire.jpg", fire_texture, xdim, ydim);
   init_texture((char *)"rock.jpg", rock_texture, xdim, ydim);
   init_texture((char *)"wood.jpg", wood_texture, xdim, ydim);
   init_texture((char *)"grass.jpg", grass_texture, xdim, ydim);
   init_texture((char *)"yellow.jpg", yellow_texture, xdim, ydim);
   init_texture((char *)"gold.jpg", gold_texture, xdim, ydim);
   init_texture((char *)"gems.jpg", gems_texture, xdim, ydim);
...
void display()
{
   ...
   block(player_col-cols/2+trim, player_row-rows/2+trim, 0+trim, 
      player_col-cols/2+1-trim, player_row-rows/2+1-trim, 1-trim);
   ...
   for (int r = 0; r < rows; r++)
      for (int c = 0; c < cols; c++)
         if (maze[r][c] == 'm')
            block(c-cols/2+trim, r-rows/2+trim, 0+trim, c-cols/2+1-trim, r-rows/2+1-trim, 1-trim);
...
void keyboard(unsigned char key, int x, int y)
{
   ...
   // Move player
   else if ((key == 'w') && (player_row+1 < rows))
   { 
      if ((maze[player_row+1][player_col] == 'm') || (maze[player_row+1][player_col] == 'g'))
      {  maze[player_row+1][player_col] = ' '; found_treasure++; }
      if (maze[player_row+1][player_col] == ' ') player_row++;
   }
   else if ((key == 's') && (player_row-1 >= 0))
   { 
      if ((maze[player_row-1][player_col] == 'm') || (maze[player_row-1][player_col] == 'g'))
      {  maze[player_row-1][player_col] = ' '; found_treasure++; }
      if (maze[player_row-1][player_col] == ' ') player_row--;
   }
   else if ((key == 'a') && (player_col-1 >= 0)) 
   { 
      if ((maze[player_row][player_col-1] == 'm') || (maze[player_row][player_col-1] == 'g'))
      {  maze[player_row][player_col-1] = ' '; found_treasure++; }
      if (maze[player_row][player_col-1] == ' ') player_col--;
   }
   else if ((key == 'd') && (player_col+1 < cols))
   { 
      if ((maze[player_row][player_col+1] == 'm') || (maze[player_row][player_col+1] == 'g'))
      {  maze[player_row][player_col+1] = ' '; found_treasure++; }
      if (maze[player_row][player_col+1] == ' ') player_col++;
   ...
   if (found_treasure == num_treasures)
   { printf("You win the game with %d treasures!!\n", found_treasure); found_treasure = 0;}
   glutPostRedisplay();
}
...
int main(int argc, char *argv[])
{
   ...
   printf("   w - move player up\n");
   printf("   a - move player left\n");
   printf("   s - move player down\n");
   printf("   d - move player right\n");
   glutMainLoop();
   return 0;
}
